...
@stop
  <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>Registro de Códigos de Retención</h1>
            </div>
            <div class="col-sm-2">
    ...
    <div class="container-fluid">
        <div class="card card-light ">
          <div class="card-header">
            <h3 class="card-title">Código de Retención</h3>
...
<!-- modal Agregar codigo de retencion -->
<div class="modal fade" id="modalAgregar">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Agregar nuevo Código de Retención</h4>
                ...
                <div class="row">  
                  <div class="col-md-6"> 
                    <div class="form-group">
                        <label>Nombre del Código de Retención:</label>   
                        <input type="text" class="form-control" id="nombren" name="nombren" placeholder="Nombre">
                    </div>    
                    <div class="form-group">
                        <label>Código:</label>   
                        <input type="text" class="form-control" id="codigon" name="codigon" placeholder="Código de Retencion">
                    </div>       
                  </div>
                </div> 
              </div>
   ...
   <!-- modal editar codigo de Retencion -->
   <div class="modal fade" id="modalEditar">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">Editar Código de Retención</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          ...
          <form id="formularioU">
              <div class="card-body">
                <div class="row">  
                  <div class="col-md-6"> 
                    <div class="form-group">
                        <label>Nombre de Código de Retención:</label>   
  ...
  <div class="modal fade" id="modalEliminar">
      <div class="modal-dialog">
            <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Eliminar un código de Retención</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
              </button>
            </div>
                  <div class="modal-body">
                    <input type="hidden" id="idD" name="idD"/> <!-- id del codigo de retencion para borrarlo -->                           
                  </div>
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>             
              <button class="btn btn-danger" id="btnBorrar" type="button" onclick="borrarcodigoret()">Borrar</button>
          </div>
        </div>      
      </div>        
  </div>
...
//************************************************************************** */
// abre el modal para editar un codigo de retencion
function abrirModalEditar(id){
  document.getElementById("formularioU").reset();   
  openLoading();
  axios.post(url+'codigoret/get_codigoret',{'id': id })
      .then((response) => {	
        closeLoading(); // cerrar loading
        if(response.data.success = 1){
          $('#modalEditar').modal('show');
          $('#idU').val(response.data.codigoret.id);
          $('#nombre').val(response.data.codigoret.nombre);    
          $('#codigo').val(response.data.codigoret.codigo);    
        }else{ 
          toastr.error('Error', 'Codigo de Retencion no encontrado'); 
        }
      })
      .catch((error) => {
        closeLoading(); // cerrar loading
...
function abrirModalAgregar(){
    ...
    // Actualizar codigo de retencion
function enviarModalEditar(){  
      ...
      axios.post(url+'codigoret/update_codigoret', formData, {  
       })
       .then((response) => {	
        closeLoading(); // cerrar loading            
...
// mensaje cuando se registra un codigoret
function mensajeResponse1(valor){
  if(valor.data.success == 1){
    toastr.success('Guardado', 'Se ha guardado el nuevo Codigo de retencion!');
    $('#modalAgregar').modal('hide'); 
  }else if(valor.data.success == 2){
    toastr.error('Error', 'Codigo de retencion NO Registrado!');
  }else{
    // error en validacion en servidor
    toastr.error('Error', 'Datos incorrectos!');
  }
...
function borrarcodigoret(){
  id = document.getElementById("idD").value;
  openLoading();// mostrar loading

  axios.post(url+'codigoret/delete_codigoret',{
    'id': id  
      })
      .then((response) => {	
        closeLoading(); // cerrar loading
        ...
        if(response.data.success == 1){
          toastr.success('Codigo de Retencion Eliminado!')
          $('#modalEliminar').modal('hide');   
        }else{
          toastr.error('Error', 'No se pudo eliminar el Codigo de Retencion');  
        }           
      })
      .catch((error) => {
        closeLoading(); // cerrar loading   
